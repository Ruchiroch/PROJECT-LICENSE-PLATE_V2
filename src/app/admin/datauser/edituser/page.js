...
export default function UserDetails() {
    ...
    useEffect(() => {
        ...
        const fetchUser = async () => {
            setShowAddButton(true); // แสดงปุ่มเพิ่มรถ
            try {
                const response = await axios.get(`/api/ad_edit?car_id=${car_id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setSubmitting(true);
            const response = await axios.put(`/api/ad_edit?car_id=${car_id}`, formData);
    ...
    const handleAddCar = (userId) => {
        router.push(`/admin/datauser/addcaruser?user_id=${userId}`);
    };
